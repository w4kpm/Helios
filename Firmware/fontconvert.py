...
#http://rinkydinkelectronics.com/r_fonts.php

chars = [[0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00], # <Space>
	 [0x18,0x3C,0x3C,0x18,0x18,0x00,0x18,0x00], # !
	 [0x66,0x66,0x24,0x00,0x00,0x00,0x00,0x00], # quote
...
char = [0x00,0x00,0xEC,0xFE,0xD6,0xD6,0xD6,0x00]
...
def invert(char):
    c = np.array(char,dtype=np.uint8)
    outputlist = np.packbits(np.fliplr(np.unpackbits(c).reshape(shape).T))
    return outputlist


print("static uint8_t font[95][8] = {")

for char in chars:
    print("{"+", ".join(["0x%x"%(x) for x in invert(char)])+"},")
print ("}")
