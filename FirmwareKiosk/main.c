...
static  float amps,windspeed,windspeedout,opamp4,snow,snowoutput,winddir;
...
static THD_FUNCTION(Thread3, arg) {
  ...
  while(TRUE)
	  ...
	  if ((b == Q_TIMEOUT) && (rx_queue_pos > 0))
		  ...
		  rx_queue_pos = 0;
		  // we have a new entry
		  chprintf((BaseSequentialStream*)&SD1,"got request %x\r\n",rx_queue_num);
...
;  
  ...
  irradiance3 = 0;
    ...
    while (TRUE)
	  ...
	  winddir = (snow/.5)*360+15.0;
	  if (windspeed < 0.5)
	      windspeed = 0;
	  else
	      windspeed = windspeed*2.237;
	  windspeedout = windspeed;
          //chprintf((BaseSequentialStream*)&SD1,"irr: %.2f  inside: %.2f ,vdd: %.2f windV: %.4f %.2fmph  snow %.2fv winddir %.2f \r\n",irradiance3,internalTemp,VDD,amps,windspeed,snow,winddir);
	  ...
	  sprintf(metrics[7],"WndDr: %3.0f",winddir);
	  /*	  if (snow < .025){
	      snowoutput = 2;
